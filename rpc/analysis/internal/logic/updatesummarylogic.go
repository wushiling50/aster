...
func (l *UpdateSummaryLogic) UpdateSummary(in *analysis.UpdateAnalysisReq) (*analysis.UpdateAnalysisResp, error) {
	...
	locksKey := l.svcCtx.Locks.GetNewLocksKey(constants.LockSummary, in.DeveloperId)
	getLock, err := l.svcCtx.Locks.TryLock(l.ctx, locksKey)
	if err != nil {
		logx.Errorf("service.UpdateSummary: Get Lock Failed: %w", err)
		resp.Base = pack.BuildBaseResp(err)
		return resp, nil
	}
...
func (l *UpdateSummaryLogic) checkIfNeedUpdate(developerId int64) (bool, error) {
	...
	if utils.CustomizeCheckIfDataExpired(summary.UpdatedAt, 7*constants.ONE_DAY) {
		return true, nil
	} else {
		return false, nil
	}
