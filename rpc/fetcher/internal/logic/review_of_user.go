...
func NewFetchReviewOfUserLogic(ctx context.Context, svcCtx *svc.ServiceContext) *FetchReviewOfUserLogic {
	return &FetchReviewOfUserLogic{
...
func (l *FetchReviewOfUserLogic) FetchReviewOfUser(userId int64, createAfter string, searchLimit int64) (err error) {
	var (
	...
	if allReview, allRepo, err = githubFunc.GetAllReviewByLogin(l.ctx, githubUser.GetLogin(), createAfter, searchLimit); err != nil {
		logx.Error(err)
		return
	}

	if err = l.rpcDelAllContributionInCategory(userId, constants.CategoryReview); err != nil {
		logx.Error(err)
		return
	}
...
func (l *FetchReviewOfUserLogic) rpcDelAllContributionInCategory(developerId int64, category string) (err error) {
